// Block-matching optical flow — the SPEC §8.3 harness core, productionized from spike
// S3 (spikes/s3-block-flow/matcher.ts; validated GO in docs/decisions.md). 16 px blocks,
// ±12 px integer SAD search, per-block variance gate. No native deps. Deterministic by
// construction: fixed scan order, search offsets radius-sorted, strict-less-than
// acceptance ⇒ nearest-radius candidate wins ties (so the matcher result is identical
// run-to-run and platform-to-platform). elapsedMs is a non-gating diagnostic only.
...
/** SPEC §8.3 defaults (validated by S3): 16 px blocks, ±12 px search, variance ≥ 100. */
